...
export interface ArcRevealHeroProps {
  /**
   * Optional custom intro content (e.g. a logo). When set, this is shown once
   * instead of cycling `greetings`.
   */
  intro?: React.ReactNode;
  /** How long custom `intro` content is held before the arc reveal (ms). */
  introHold?: number;
  /** Greetings cycled before the arc reveal. Ignored when `intro` is set. */
...
export function ArcRevealHero({
  ...
  // Single intro hold (logo) or greeting cycle.
  React.useEffect(() => {
    ...
    return () => window.clearTimeout(t);
  }, [phase, index, greetingHold, greetings.length, intro, introHold]);
      ...
      <AnimatePresence>
        {showOverlay && (
          <motion.div
            ...
            className={cn(
              // z-[60] sits above the fixed site header (z-50) during intro
              "absolute inset-x-0 top-0 z-[60] h-screen overflow-hidden bg-foreground",
              introClassName,
            )}
          >
            {/* Intro: custom content (logo) or cycled greetings */}
              ...
              <AnimatePresence mode="wait">
                {phase === "intro" && intro && (
                  <motion.div
                    ...
                    className="flex select-none items-center justify-center px-6"
                  >
                    {intro}
                  </motion.div>
                )}
                {phase === "intro" && !intro && current && (
                  <motion.span
